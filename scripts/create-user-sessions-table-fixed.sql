...
CREATE POLICY "Users can create their own login history" ON user_login_history
    FOR INSERT WITH CHECK (auth.uid() = user_id);

-- Create function to log user activity (if user_activities table exists)
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_activities') THEN
        ...
        RETURN activity_id;
    ELSE
        -- Return a dummy UUID if table doesn't exist
...
BEGIN
    ...
    IF p_login_status = 'success' THEN
        ...
        VALUES (
            ...
            NOW() + INTERVAL '30 days' -- 30 day session
        )
    ...
    PERFORM log_user_activity(
        ...
        'login', 
        CASE 
            WHEN p_login_status = 'success' THEN 'User logged in successfully'
...
BEGIN
    ...
    PERFORM log_user_activity(
        ...
        'logout', 
        'User logged out',
        'User logged out. Reason: ' || p_reason
    );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
-- Success message
SELECT 'User sessions and login tracking tables created successfully! 🎉' as status; 