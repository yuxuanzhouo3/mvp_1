-- Sync email addresses from auth.users to profiles table (FIXED VERSION)
...
-- (Removed username column since it doesn't exist)
...
-- Show users that were synced (removed username column)
SELECT 
    p.id,
    p.email,
    p.full_name,
