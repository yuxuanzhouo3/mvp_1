  ...
  return (
    <>
      ...
      <div className={`hidden md:block ${open ? 'w-64' : 'w-20'} shrink-0 transition-all duration-200`}>
        ...
        <div className={`flex flex-col ${open ? 'w-64' : 'w-20'} h-[calc(100vh-73px)] bg-background border-r transition-all duration-200 fixed top-[73px] left-0 z-50 overflow-y-auto overscroll-contain`}>
      <div className="flex items-center justify-center h-16 border-b px-4">
          <h1 className={`text-xl font-bold text-primary transition-opacity duration-200 ${open ? 'opacity-100' : 'opacity-0 w-0 overflow-hidden'}`}>{getBrandName({ isCN: isChinaDeployment() })}</h1>
      </div>
